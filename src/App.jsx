...
function AdminApp() {
  ...
  const createGoogleCalendarEvent = async (eventData, user) => {
    const token = localStorage.getItem("google_token");
    if (!token) {
      setGoogleAuthError("Brak połączenia z Google Calendar");
      return null;
    }

    try {
      // Проверяем токен
      const isValid = await checkGoogleToken(token);
      if (!isValid) {
        setGoogleAuthError("Sesja Google wygasła. Odśwież połączenie.");
        return null;
      }
      ...
      if (!response.ok) {
        ...
        console.error("Ошибка Google Calendar API:", errorData);
        throw new Error(errorData.error?.message || "Błąd Google Calendar");
      }
      ...
      setGoogleAuthError("Nie udało się utworzyć wydarzenia w Google Calendar");
      return null;
    }
  };
  ...
  const publishBulkEvents = async () => {
    if (bulkForm.selectedEvents.length === 0) {
      alert("Wybierz wydarzenia do publikacji");
      return;
    }

    if (!isAuthorized) {
      alert("Aby publikować, musisz połączyć się z Google Calendar");
      return;
    }
    ...
    for (const eventId of bulkForm.selectedEvents) {
      // Ищем событие в confirmedEvents (Oczekują)
    ...
    // Удаляем опубликованные из confirmedEvents (Oczekują)
    ...
    alert(`✅ Opublikowano ${successCount} zmian, ${failCount} błędów`);
  };
  ...
  const handleCreateEvent = async () => {
    ...
    if (bulkMode) {
      // РЕЖИМ МАССОВОЙ ПУБЛИКАЦИИ - добавляем в confirmedEvents (Oczekują)
      const newConfirmedEvents = eventForm.userIds.map((userId) => {
        const user = users.find((u) => u.id === userId);
        return {
          ...eventForm,
          userId,
          id: `confirmed_${Date.now()}_${userId}_${Math.random()
            .toString(36)
            .substr(2, 6)}`,
          createdAt: new Date().toISOString(),
          isPending: true,
      ...
      // Сохраняем в confirmedEvents (это и есть список "Oczekują")
      await saveConfirmedEvents([...confirmedEvents, ...newConfirmedEvents]);

      // Показываем уведомление
      alert(
        `✅ Dodano ${newConfirmedEvents.length} zmian do listy oczekujących`
      );
    } else {
      // ОБЫЧНЫЙ РЕЖИМ - сразу публикуем в Google Calendar
      const createdEvents = [];
      let successCount = 0;
      ...
      if (createdEvents.length > 0) {
        ...
        alert(`✅ Opublikowano ${successCount} zmian w Google Calendar`);
      }
    }
  ...
  const saveConfirmedEvents = async (newConfirmedEvents) => {
    console.log("Сохранение confirmedEvents (Oczekują):", newConfirmedEvents);
    ...
    try {
      ...
      console.log("ConfirmedEvents (Oczekują) сохранены успешно");
    } catch (error) {
          ...
          <div className="google-status">
            {isAuthorized ? (
              <>
                ...
                >
                  ...
                  {tokenExpiryTime && (
                    <span className="token-expiry">
                      {new Date(tokenExpiryTime).toLocaleTimeString("pl-PL", {
                        hour: "2-digit",
                        minute: "2-digit",
                      })}
                    </span>
                ...
                {/* Кнопка обновления */}
                <button
                  className="btn-google-refresh"
                  onClick={refreshGoogleConnection}
                  disabled={googleConnecting}
                  title="Odśwież połączenie z Google"
                >
                  {googleConnecting ? (
                    ...
                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                      <path
                        d="M13 8C13 10.7614 10.7614 13 8 13C5.23858 13 3 10.7614 3 8C3 5.23858 5.23858 3 8 3C9.63505 3 11.0881 3.78555 12 4.9995M12 4.9995V2.5M12 4.9995H9.5"
                        stroke="currentColor"
                        strokeWidth="1.5"
                        strokeLinecap="round"
                      />
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
                    <path
                      d="M4 8H12M12 8L10 6M12 8L10 10M8 3V5M8 11V13M5 5L4 4M11 11L12 12M3 8H1M15 8H13"
                      stroke="currentColor"
                      strokeWidth="1.5"
                      strokeLinecap="round"
                    />
              ...
              <>
                ...
                {googleAuthError && (
                  <span className="google-error">{googleAuthError}</span>
                )}
              </>
            )}
          </div>
